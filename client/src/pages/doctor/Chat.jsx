...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        const [patientsRes, allMessagesRes] = await Promise.all([
        ...
        if (savedId) {
          const found = patientsData.find((patient) => patient._id === savedId);
  ...
  const openChat = (patient) => {
    ...
    setShowChat(true);
    localStorage.setItem("selectedChatUser", patient._id);
    setUnread((prev) => ({ ...prev, [patient._id]: 0 }));
    API.patch(`/messages/${patient._id}/seen`).catch(() => {});
  };
  ...
  const lastMsgByPatient = (patientId) =>
    allMessages
      .filter((message) => {
        const senderId = message.sender?._id || message.sender;
        const receiverId = message.receiverId?._id || message.receiverId;
        return senderId === patientId || receiverId === patientId;
      })
      .sort(
  ...
  const filteredPatients = patients.filter((patient) =>
    patient.name?.toLowerCase().includes(search.toLowerCase().trim()),
  );

  const PatientList = (
    <div className='flex h-full min-h-0 w-full flex-col border-r border-slate-700 bg-slate-900 md:w-1/3 lg:w-1/4'>
      <div className='border-b border-slate-700 p-4'>
        <h1 className='text-lg font-semibold text-white'>Patient Chat</h1>
        <p className='mt-1 text-sm text-slate-400'>
          Patients ke saath direct conversation yahin manage karo.
        </p>
      ...
      <div className='min-h-0 flex-1 overflow-y-auto'>
        ...
        : filteredPatients.length === 0 ?
          <div className='p-6 text-center text-sm text-slate-400'>
            {patients.length === 0 ?
              "No patients available for chat yet."
            : "No patient matched your search."}
          </div>
        : filteredPatients.map((patient) => {
            const lastMessage = lastMsgByPatient(patient._id);
            const isSelected = selectedUser?._id === patient._id;
            const isOnline = onlineUsers.includes(patient._id);
    ...
    <DashboardLayout>
      <div className='flex flex-col gap-4'>
        <div>
          ...
          <p className='mt-1 text-sm text-slate-400'>
            Patients ke saath direct conversation manage karo.
          </p>
        </div>
        ...
        <div className='h-[calc(100dvh-10rem)] min-h-128 overflow-hidden rounded-xl border border-slate-700 bg-slate-900 md:h-[calc(100vh-10rem)]'>
          <div className='flex h-full min-h-0 w-full md:hidden'>
            {!showChat ? PatientList : ChatWindow}
          </div>
