...
const Steps = () => {
  ...
  const goalPct =
    today ? Math.min(Math.round((today.steps / today.goal) * 100), 100) : 0;
        ...
        {pending && (
          <div className='bg-yellow-500/10 border border-yellow-500/40 rounded-xl p-4 space-y-3'>
            <p className='text-white font-medium'>
              🎯 Coach{" "}
              <span className='text-yellow-400'>{pending.coachId?.name}</span>{" "}
              assigned a step target
            </p>
            <p className='text-slate-300 text-sm'>
              Target:{" "}
              <span className='text-white font-bold'>
                {pending.goal?.toLocaleString()} steps
              </span>
            </p>
          ...
          <div className='bg-slate-800 border border-slate-700 rounded-xl p-4 space-y-3'>
            <div className='flex justify-between text-sm'>
              ...
              <span className='text-white'>
                {today.steps.toLocaleString()} / {today.goal.toLocaleString()}
              </span>
            </div>
            ...
            <p
              className={`text-sm font-medium ${goalPct >= 100 ? "text-green-400" : "text-slate-400"}`}>
