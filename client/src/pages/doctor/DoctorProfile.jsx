...
import { FaUserMd, FaCalendarAlt, FaFileMedical, FaPrescription, FaComments, FaVideo, FaPhone, FaMapMarkerAlt, FaStar, FaRupeeSign, FaClock, FaStethoscope } from "react-icons/fa";
...
const DoctorProfile = () => {
  ...
  const plans = [
    ...
    },
    {
      ...
      price: 999,
      features: ["4 Consultations", "Priority Chat", "30 Days Access", " prescriptions Included"],
      popular: true,
    },
    {
      id: "enterprise",
      name: "Full Care Plan",
      price: 2999,
      features: ["Unlimited Consultations", "24/7 Chat", "90 Days Access", "Lab Report Review", "Medication Management"],
      popular: false,
    },
  ];
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-8 overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 shadow-xl">
          <div className="relative h-32 bg-gradient-to-r from-blue-600 to-purple-600 md:h-48">
            <div className="absolute -bottom-12 left-8">
              <div className="h-24 w-24 rounded-2xl border-4 border-slate-950 bg-slate-200 object-cover md:h-32 md:w-32">
                {doctor.photo ? (
                  <img src={doctor.photo} alt={doctor.name} className="h-full w-full rounded-2xl object-cover" />
                ) : (
          ...
          <div className="pt-16 pb-8 pl-8 pr-8 md:pl-40">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-blue-400">{doctor.specialty || "General Physician"}</p>
            ...
            {doctor.qualifications && (
              <div className="mt-6">
                <h3 className="mb-2 font-semibold text-white">Qualifications</h3>
                <div className="flex flex-wrap gap-2">
                  {doctor.qualifications.map((qual, idx) => (
                    <span key={idx} className="rounded-lg bg-slate-800 px-3 py-1 text-sm text-slate-300">
                      {qual}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {doctor.services.map((service, idx) => (
                    <span key={idx} className="rounded-lg bg-blue-500/20 px-3 py-1 text-sm text-blue-300">
                      {service}
                    </span>
                  ))}
                </div>
            ...
            {plans.map((plan) => (
              ...
              >
                ...
                <div className="my-4 flex items-baseline gap-1">
                  ...
                  <span className="text-slate-400">/ {plan.id === "basic" ? "consult" : "month"}</span>
                </div>
        ...
        <div className="rounded-2xl border border-slate-700 bg-slate-900 p-6">
          ...
          <button className="mt-6 w-full rounded-xl bg-orange-500 py-4 font-semibold text-white hover:bg-orange-600 transition">
            Book Appointment - ₹{selectedPlan === "basic" ? 499 : selectedPlan === "premium" ? 999 : 2999}
          </button>
        </div>
