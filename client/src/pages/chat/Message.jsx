...
const Message = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        const [meRes, allMessagesRes] = await Promise.all([
          API.get("/auth/me"),
        ...
        const { assignedCoach, coachId } = meRes.data?.data || {};
        const coach = assignedCoach || coachId;
        const preparedContacts = coach ? [coach] : [];
        ...
        if (savedId) {
          const found = preparedContacts.find(
            (contact) => contact._id === savedId,
          );
        ...
        } else if (preparedContacts.length > 0) {
          setSelectedUser(preparedContacts[0]);
          setShowChat(true);
        }
      } catch (err) {
        console.error(err);
        if (isMounted) {
          setError("Failed to load chat contact.");
        }
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  useEffect(() => {
    ...
    const handleMessagesSeen = ({ userId }) => {
      const seenBy = normalizeId(userId);

      const markSeen = (message) => {
        const receiverId = normalizeId(
          message.receiverId?._id || message.receiverId,
        );
        if (receiverId === seenBy) {
          return { ...message, seen: true };
        }
        return message;
      };

      setMessages((prev) => prev.map(markSeen));
      setAllMessages((prev) => prev.map(markSeen));

      if (
        selectedUser &&
        normalizeId(selectedUser._id) === seenBy
      ) {
        fetchConversation();
      }
    };
  ...
  const openChat = (contact) => {
    ...
    setShowChat(true);
    localStorage.setItem("selectedChatUser", contact._id);
    setUnread((prev) => ({ ...prev, [contact._id]: 0 }));
    API.patch(`/messages/${contact._id}/seen`).catch(() => {});
  };
  ...
  const lastMsgByClient = (contactId) =>
    allMessages
      .filter((message) => {
        const senderId = message.sender?._id || message.sender;
        const receiverId = message.receiverId?._id || message.receiverId;
        return senderId === contactId || receiverId === contactId;
      })
      .sort(
    ...
    <div className='flex h-full min-h-0 w-full flex-col border-r border-slate-700 bg-slate-900 md:w-1/3 lg:w-1/4'>
      <div className='border-b border-slate-700 p-4'>
        ...
        <p className='mt-1 text-sm text-slate-400'>
          Your assigned coach ke saath direct messsaging.
        </p>
      ...
      <div className='min-h-0 flex-1 overflow-y-auto'>
        ...
        : filteredContacts.length === 0 ?
          <div className='p-6 text-center text-sm text-slate-400'>
            {contacts.length === 0 ?
              "Aapke pass abhi koi assigned coach nahi hai."
            : "No contact matched your search."}
          </div>
        : filteredContacts.map((contact) => {
            const lastMessage = lastMsgByClient(contact._id);
      ...
      : <div className='flex flex-1 items-center justify-center bg-slate-950/20 p-6 text-center text-sm text-slate-400'>
          <div>
            <p className='font-medium text-slate-300'>
              Select a coach to start chatting
            </p>
            <p className='mt-1'>
    ...
    <DashboardLayout>
      <div className='flex flex-col gap-4'>
        <div>
          ...
          <p className='mt-1 text-sm text-slate-400'>
            Aapke assigned coach ke saath live messaging pane.
          </p>
        </div>
