...
import { FaCalendarAlt, FaClock, FaVideo, FaComments, FaMapMarkerAlt, FaRupeeSign, FaCheck } from "react-icons/fa";
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-950">
        <div className="max-w-md rounded-2xl border border-green-500/30 bg-slate-900 p-8 text-center">
          ...
          <h2 className="mb-2 text-2xl font-bold text-white">Appointment Booked!</h2>
          <p className="mb-6 text-slate-300">
            Your appointment with Dr. {doctor?.name} is confirmed.
          </p>
          <Link to="/notifications" className="rounded-xl bg-orange-500 px-6 py-3 font-semibold text-white hover:bg-orange-600 transition">
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-4 md:p-8">
      <div className="mx-auto max-w-4xl">
        <h1 className="mb-8 text-3xl font-bold text-white">Book Appointment</h1>

        {!doctor ? (
          <div className="rounded-2xl border border-slate-700 bg-slate-900 p-8 text-center">
            <p className="text-slate-400">Doctor not found. Please select a doctor from the directory.</p>
            <Link to="/doctors" className="mt-4 inline-block rounded-xl bg-orange-500 px-6 py-3 font-semibold text-white hover:bg-orange-600">
              Browse Doctors
            </Link>
          </div>
        ) : (
          <div className="grid gap-8 md:grid-cols-3">
              ...
              <div className="sticky top-8 rounded-2xl border border-slate-700 bg-slate-900 p-6">
                <div className="flex items-center gap-4">
                  <div className="h-16 w-16 rounded-xl bg-slate-800">
                    {doctor.photo ? (
                      <img src={doctor.photo} alt={doctor.name} className="h-full w-full rounded-xl object-cover" />
                    ) : (
            ...
            <div className="md:col-span-2">
              <div className="rounded-2xl border border-slate-700 bg-slate-900 p-6">
                ...
                <div className="mb-6 grid gap-4 md:grid-cols-3">
                  {plans.map((plan) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-slate-400">{plan.sessions} session{plan.sessions > 1 ? "s" : ""}</p>
                    </label>
                  ))}
                </div>

                <h2 className="mb-4 text-xl font-bold text-white">Consultation Mode</h2>
                <div className="mb-6 flex gap-4">
                  ...
                  >
                    <input
                      ...
                      className="sr-only"
                    />
                    <FaVideo className={consultationMode === "video" ? "text-blue-400" : "text-slate-500"} />
                    <span className={consultationMode === "video" ? "font-semibold text-white" : "text-slate-400"}>
                      Video Call
                    </span>
                  </label>
                  <label
                  ...
                  >
                    <input
                      ...
                      className="sr-only"
                    />
                    <FaComments className={consultationMode === "chat" ? "text-green-400" : "text-slate-500"} />
                    <span className={consultationMode === "chat" ? "font-semibold text-white" : "text-slate-400"}>
                      Chat
                    </span>
                  </label>
                  <label
                  ...
                  >
                    <input
                      ...
                      className="sr-only"
                    />
                    <FaMapMarkerAlt className={consultationMode === "in_person" ? "text-purple-400" : "text-slate-500"} />
                    <span className={consultationMode === "in_person" ? "font-semibold text-white" : "text-slate-400"}>
                      In-Person
                    </span>
                  </label>
                </div>

                <h2 className="mb-4 text-xl font-bold text-white">Select Date & Time</h2>
                <div className="mb-6">
                ...
                <div className="mb-8">
                  <label className="mb-2 block text-sm text-slate-400">Available Slots</label>
