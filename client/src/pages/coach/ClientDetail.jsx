...
const ClientDetail = () => {
  ...
  const fetchSteps = () =>
    API.get(`/steps/client/${userId}`).then((res) =>
      setStepsRecords(res.data.data || []),
    );
  ...
  return (
    <DashboardLayout>
      <div className='w-full max-w-4xl mx-auto space-y-6 px-3 overflow-x-hidden'>
        {/* Header */}
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 gap-3 w-full'>
          {[
            {
              label: "Goal",
              value: client.goal || "N/A",
              color: "text-blue-400",
            },
            {
              label: "Weight",
              value: client.weight ? `${client.weight} kg` : "N/A",
            ...
            },
            {
              label: "Step Goal %",
              value: `${goalPct}%`,
              color: goalPct >= 100 ? "text-green-400" : "text-orange-400",
            },
          ].map((s) => (
            <div
              key={s.label}
              ...
              <p className={`text-lg font-bold capitalize ${s.color}`}>
                {s.value}
              </p>
            </div>
          ))}
        ...
        <div className='bg-slate-800 border border-slate-700 rounded-xl p-4'>
          <div className='flex justify-between text-sm mb-2'>
            ...
            <span className='text-white'>
              {todayCount.toLocaleString()} / {stepGoal.toLocaleString()}
            </span>
          </div>
            ...
            <div className='bg-slate-800 border border-slate-700 rounded-xl p-4'>
              ...
              <div className='space-y-2'>
                ...
                : stepsRecords.map((r) => {
                    ...
                    return (
                      <div className='flex flex-col sm:flex-row items-center justify-between bg-slate-700 rounded-lg px-3 py-2 gap-1'>
                        ...
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${
                            r.goalStatus === "pending" ?
                              "bg-yellow-500/20 text-yellow-400"
                            : r.goalStatus === "rejected" ?
                  ...
                  <div className='flex justify-between items-start'>
                    <div>
                      ...
                      <p className='text-slate-400 text-xs capitalize'>
                        {w.type} · {w.exercises?.length || 0} exercises
                      </p>
                    </div>
